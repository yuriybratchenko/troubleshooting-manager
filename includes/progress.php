...
if ( ! class_exists( 'Troubleshooting_Manager_Progress' ) ) {

	/**
	 * Define Troubleshooting_Manager_Progress class
	 */
	class Troubleshooting_Manager_Progress {

		/**
		 * [$meta_slug description]
		 * @var string
		 */
		private $meta_slug = 'troubleshooting_manager_learning_progress';
		...
		public function get_course_progress_data( $course_id ) {
			...
			$query = new WP_Query( [
				...
				'tax_query' => [
					[
						'taxonomy'	=> troubleshooting_manager()->post_type->course_term_slug(),
						'field'		=> 'term_id',
						'terms'		=> $course_id,
					]
				]
			...
			if ( in_array( 'guest', $progress_data ) ) {
				$link = get_term_link( (int)$course_id, troubleshooting_manager()->post_type->course_term_slug() );
		...
		/**
		 * [is_course_article description]
		 * @param  string  $post_id [description]
		 * @return boolean          [description]
		 */
		public function is_course_article( $post_id = '' ) {

			return has_term( '', troubleshooting_manager()->post_type->course_term_slug(), $post_id );
		}
