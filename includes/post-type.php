...
if ( ! class_exists( 'Troubleshooting_Manager_Post_Type' ) ) {

	/**
	 * Define Troubleshooting_Manager_Post_Type class
	 */
	class Troubleshooting_Manager_Post_Type {
		...
		public function register_post_type() {
			...
			$post_types = [
				...
				$this->article_post_slug() => [
					'labels'              => [
						'name'          => esc_html__( 'Articles', 'troubleshooting-manager' ),
						'singular_name' => esc_html__( 'Article', 'troubleshooting-manager' ),
						'all_items'     => esc_html__( 'All Articles', 'troubleshooting-manager' ),
		...
		public function register_taxonomy() {
			...
			$taxonomy_list = [
				...
				$this->category_term_slug() => [
					'labels'        => [
						'name'          => esc_html__( 'Arcticle Category', 'troubleshooting-manager' ),
						'singular_name' => esc_html__( 'Arcticle Category', 'troubleshooting-manager' ),
						'label'         => esc_html__( 'Arcticle Category', 'troubleshooting-manager' ),
						'menu_name'     => esc_html__( 'Arcticle Category', 'troubleshooting-manager' ),
						'search_items'  => esc_html__( 'Search Article Categories', 'troubleshooting-manager' ),
					],
					'hierarchical'          => true,
					'rewrite'               => true,
					'query_var'             => true,
		...
		public function article_view_template( $template ) {
			...
			if ( is_single() && $this->article_post_slug() === get_post_type() ) {
				...
				$find[] = $file;
				$find[] = troubleshooting_manager()->template_path() . $file;
			...
			if ( $file ) {
				...
				if ( ! $template ) {
					$template = troubleshooting_manager()->plugin_path( 'templates/' . $file );
				}
			}
		...
		public function admin_filter_by_tax( $post_type, $which ) {
			...
			echo '<option value="">' . sprintf( esc_html__( 'Show All %s', 'troubleshooting-manager' ), $taxonomy_name ) . '</option>';
