...
abstract class Troubleshooting_Manager_Base extends Widget_Base {
	...
	public function __get_global_template( $name = null ) {
		...
		if ( ! $template ) {
			$template = croco_extra()->get_template( $this->get_name() . '/global/' . $name . '.php' );
		}
	...
	public function __get_render_template( $name = null ) {
		return croco_extra()->get_template( $this->get_name() . '/render/' . $name . '.php' );
	}
	...
	public function __get_edit_template( $name = null ) {
		return croco_extra()->get_template( $this->get_name() . '/edit/' . $name . '.php' );
	}
	...
	public function __open_wrap() {
		printf( '<div class="elementor-%s troubleshooting-manager-addons">', $this->get_name() );
	}
